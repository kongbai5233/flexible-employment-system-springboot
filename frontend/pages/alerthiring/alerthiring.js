// pages/alerthiring/alerthiring.js
...
Page({
  ...
  async zhaopin(){
    ...
    if(this.data.alertImg!=0)uploadImg = await this.uploadAllImages(imgArry);
    else {
      for(let url of imgArry){
        let fileNameWithExt = url.split("/upload/")[1]; 
        uploadImg.push(fileNameWithExt)
      }
    }
    this.setData({
      uploadImg
...
// console.log(uploadImg);
   ...
   wx.request({
     url: 'http://127.0.0.1:8080/order/alertOrder',
     method:"POST",
     header: {
  ...
  upphoto() {
    ...
    wx.chooseMedia({
      ...
      success(res) {
        // console.log(res);
        let img=[];
        for(let a of res.tempFiles){
          img.push(a.tempFilePath)
        }
        // console.log(img);
        that.setData({
          imgArry: img
  ...
  onLoad(options) {
    ...
    this.setData({
      ...
      wx.request({
        url: 'http://127.0.0.1:8080/order/getmerchat?staffID='+options.staffid,
        success(res){
          console.log(res);
          that.setData({
            Merchant:res.data
      ...
      console.log(openid);
      wx.request({
        url: "http://127.0.0.1:8080/order/getinfo?openid="+openid+"&orderId="+options.orderId,
        success(res){
          console.log(res);
          that.setData({
          demand: res.data.demand,
          imgArry: res.data.imgArry,
